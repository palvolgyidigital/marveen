...
// RE-ENABLED 2026-09-04, in the commit that adds scripts/hooks/tudastagadas-gate.py.
// Why it was skipped: injectTudastagadasGate calls isUnsafeHookCommand(), which
// returns true -- and the injector then no-ops -- for a hook command whose script
// path does not exist on disk (agent-scaffold.ts:148, `!existsSync(m[0])`). With
// the script uncommitted, these three tests passed only in a working tree that
// happened to have it sitting on disk, and failed on any clean checkout. Now the
// script is tracked, so the fixture gap is closed and the .skip is gone.
